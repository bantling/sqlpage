...
DECLARE
  ...
  -- True for personal customers, false for business customers
  V_IS_PERSONAL BOOL;

  -- Personal customers don't always have addresses
  V_PERSONAL_HAS_ADDRESS BOOL;

  -- Business address type relids (null for personal addresses)
  V_BUSINESS_ADDRESS_TYPE_RELIDS BIGINT[];

  -- Address loop counter lower bound
  V_LOOP_ADDRESS_LOWER INT;

  -- Address loop counteR upper bound
  V_LOOP_ADDRESS_UPPER INT;
  ...
  C_FIRST_MIDDLE_NAMES TEXT[][] = ARRAY[[
    'Anna'
   ,'Britney'
   ,'Christie'
   ,'Denise'
...
BEGIN
  ...
  FOR V_COUNT_CUSTOMERS IN 1 .. C_NUM_CUSTOMERS LOOP
    ...
    RAISE NOTICE 'V_IS_PERSONAL                  = %', V_IS_PERSONAL;

    V_PERSONAL_HAS_ADDRESS = NULL;
    V_BUSINESS_ADDRESS_TYPE_RELIDS = NULL;
    V_BUSINESS_NAME = NULL;

    -- A loop cannot have a null lower or upper bound
    -- In the case where have a person customer with no address, use bounds 1, 0
    -- Counting up from 1 to 0 causes the loop to not execute at all, generating no addresses.
    V_LOOP_ADDRESS_LOWER = 1;
    V_LOOP_ADDRESS_UPPER = 0;
    ...
    ELSE
      ...
      -- The number of addresses to generate is the number address types
      V_LOOP_ADDRESS_UPPER = ARRAY_LENGTH(V_BUSINESS_ADDRESS_TYPE_RELIDS, 1);

      -- A single business is related to each of its addresses
      V_BUSINESS_NAME = C_BUSINESS_NAMES[managed_code.RANDOM_INT(1, ARRAY_LENGTH(C_BUSINESS_NAMES, 1))];
      RAISE NOTICE 'V_BUSINESS_NAME                = %', V_BUSINESS_NAME;
      ...
      RETURNING relid
           INTO V_BUSINESS_RELID;
    END IF; -- IF V_PERSONAL

    -- Loop 1 time for a person that has an address; for each selected address type for a business
    FOR V_LOOP_ADDRESS IN V_LOOP_ADDRESS_LOWER .. V_LOOP_ADDRESS_UPPER LOOP
      ...
      INSERT INTO managed_tables.address(
       ...
       ,mailing_code
        ...
        V_DESCRIPTION
       ,V_BUSINESS_ADDRESS_TYPE_RELIDS[V_LOOP_ADDRESS]
       ,V_COUNTRY_RELID
       ,V_REGION_RELID
       ,V_CITY
       ,V_ADDRESS_LINE_1
      ...
      ELSE
        -- Relate the address to the business
        INSERT INTO managed_tables.customer_business_address_jt(
          business_relid
         ,address_relid
        ) VALUES (
          V_BUSINESS_RELID
